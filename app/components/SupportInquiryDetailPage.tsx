  ...
  return (
    <div className="container mx-auto px-6 py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" className="border-primary/30" onClick={() => nav(-1)}>목록으로</Button>
      </div>
      ...
      <Card className="border-primary/20 mx-auto" style={{ maxWidth: 720 }}>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm">유형: <span className="font-medium">{rec.kind}</span></div>
          {rec.game && <div className="text-sm">게임: <span className="font-medium">{rec.game}</span></div>}
          <div className="text-sm">상태: {status === "답변완료" ? (
            <Badge className="bg-green-500/20 text-green-400 border-green-500/30">{status}</Badge>
          ) : status === "처리중" ? (
            <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30">{status}</Badge>
          ) : (
            <Badge variant="secondary" className="bg-primary/10 text-primary border border-primary/20">{status}</Badge>
          )}
          </div>

          {rec.types && Array.isArray(rec.types) && (
            <div className="text-sm">문제 유형: {(rec.types as any[]).join(", ")}</div>
          )}
          {rec.reason && <div className="text-sm">환불 사유: {String(rec.reason)}</div>}
          {rec.orderId && <div className="text-sm">주문번호: {String(rec.orderId)}</div>}
          {rec.desc && (
          ...
          <div className="text-xs text-muted-foreground">접수 일시: {rec.createdAt ? new Date(rec.createdAt).toLocaleString("ko-KR") : "-"}</div>
        </CardContent>
      </Card>
    </div>
  );
