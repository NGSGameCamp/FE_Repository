...
export default function ProfilePage() {
  ...
  const followingBoards: string[] = useMemo(() => {
    try {
      return JSON.parse(localStorage.getItem("community:follows") || "[]");
    } catch {
      return [];
    }
  }, []);
  ...
  const followingGames: string[] = useMemo(() => {
    try {
      return JSON.parse(localStorage.getItem("followingGames") || "[]");
    } catch {
      return [];
    }
  }, []);
  ...
  const myPosts: any[] = useMemo(() => {
    try {
      ...
      return Array.isArray(raw)
        ? raw.filter((p: any) => p.author === me).slice(0, 5)
        : [];
    } catch {
      return [];
    }
  ...
  return (
    <div className="container mx-auto px-6 py-6 space-y-6">
        ...
        <CardContent className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <Button
              size="sm"
              variant="outline"
              className="border-primary/30 mt-2"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="text-base">팔로우 게임</CardTitle>
          </CardHeader>
          <CardContent className="text-sm space-y-2">
            ...
            {followingGames.map((g) => (
              <div key={g} className="rounded-md border border-primary/20 p-2">
                {g}
              </div>
            ))}
          </CardContent>
        ...
        <Card className="border-primary/20">
          ...
          <CardContent className="text-sm space-y-2">
            {myInquiries.length === 0 && (
              <div className="text-muted-foreground">문의 내역이 없습니다.</div>
            )}
            {myInquiries.map((inq) => (
              <Link
                key={inq.id}
        ...
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="text-base">팔로잉 유저</CardTitle>
          </CardHeader>
          <CardContent className="text-sm space-y-2">
            ...
            {followingBoards.map((b) => (
              <div key={b} className="rounded-md border border-primary/20 p-2">
                {b}
              </div>
            ))}
          </CardContent>
        ...
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="text-base">구독 중인 배급사</CardTitle>
          </CardHeader>
          <CardContent className="text-sm space-y-2">
            {followingBoards.length === 0 && (
              <div className="text-muted-foreground">
        ...
        <Card className="border-primary/20 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">최근 작성한 포스트</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            ...
            {myPosts.map((p) => (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">
                  {new Date(p.date || Date.now()).toLocaleString("ko-KR")}
                </div>
              </div>
            ))}
          </CardContent>
