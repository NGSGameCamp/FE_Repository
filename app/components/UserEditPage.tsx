...
export default function UserEditPage() {
  ...
  useEffect(() => {
    ...
    try {
      setPhone(localStorage.getItem(`profile:phone:${user?.email}`) || "010-1234-5678");
    } catch {
      setPhone("010-1234-5678");
    }
    try {
      setBirth(localStorage.getItem(`profile:birth:${user?.email}`) || "1996-03-14");
    } catch {
      setBirth("1996-03-14");
    }
      ...
      <Card className="w-full max-w-md shadow-xl border border-primary/10 bg-card/90">
        ...
        <CardContent className="flex flex-col gap-6 px-8 pb-8">
          <div className="space-y-4">
            ...
            <EditRow label="생년월일">
              <Input id="birth" value={birth} onChange={e => setBirth(e.target.value)} placeholder="YYYY-MM-DD" type="date" />
            </EditRow>
          </div>
