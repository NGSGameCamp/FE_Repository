...
import { HelpCircle, ShieldCheck, Info, User, CreditCard, ShieldAlert, MessageSquare, Cog, MoreHorizontal } from "lucide-react";
...
const detailCategories: { id: DetailCategory; desc: string; alert?: "주의" | "보안" }[] = [
...
export default function SupportOneToOnePage() {
  ...
  const submit = () => {
    ...
    if (desc.trim().length < 10) return setError("상세 내용을 10자 이상 입력하세요.");
    try {
      ...
      navigate("/support/success", { state: { id: `INQ-${id}`, kind: "1:1 문의" } });
    } catch {
    ...
    <div className="container mx-auto px-6 py-6 space-y-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">개인적인 문의나 계정 관련 문제를 안전하게 상담받을 수 있습니다. 민감한 정보는 보안 처리됩니다.</p>
        </div>
      </div>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-green-500/30 bg-green-500/5 p-4">
            <div className="font-medium flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-green-400" /> 보안 조치
            </div>
          ...
          {detailCategories.map((c) => (
            <button
              ...
              className={`text-left rounded-lg border p-3 transition ${
                selected === c.id ? "border-primary bg-primary/5" : "border-primary/20 hover:border-primary/40"
              }`}
            >
              <div className="flex items-center gap-2">
                ...
                {c.alert === "보안" && (
                  <Badge className="ml-auto bg-red-500/20 text-red-400 border-red-500/30">보안</Badge>
                )}
                {c.alert === "주의" && (
                  <Badge className="ml-auto bg-amber-500/20 text-amber-300 border-amber-500/30">주의</Badge>
                )}
              </div>
