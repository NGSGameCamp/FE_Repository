...
const subTypes: { id: SubType; desc: string; icon: JSX.Element }[] = [
  ...
  {
    id: "파트너십",
    desc: "비즈니스 협업/파트너십 제안",
    icon: <Briefcase className="h-5 w-5" />,
  },
  {
    id: "언론/미디어",
    desc: "보도자료, 인터뷰, 미디어 문의",
    icon: <Newspaper className="h-5 w-5" />,
  },
  {
    id: "기술 문의",
    desc: "API, 개발자 지원, 기술적 질문",
    icon: <Cpu className="h-5 w-5" />,
  },
  {
    id: "커뮤니티",
    desc: "이벤트/커뮤니티 활동/사용자 모임",
    icon: <Users className="h-5 w-5" />,
  },
  {
    id: "콘텐츠 문의",
    desc: "게임 리뷰/제작/스트리밍 관련",
...
export default function SupportOtherInquiryPage() {
  ...
  const submit = () => {
    ...
    if (desc.trim().length < 10)
      return setError("문의 내용을 10자 이상 입력하세요.");
    try {
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            게임이나 계정 외 다양한 문의사항을 접수하실 수 있습니다. 적절한 담당
            부서로 전달해 드립니다.
          </p>
        </div>
      </div>
      ...
      <Card className="border-primary/20">
        ...
        <CardContent className="space-y-6">
          <Textarea
            ...
            placeholder="문의하실 내용을 자세히 설명해주세요"
          />
