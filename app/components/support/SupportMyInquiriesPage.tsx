...
function loadAll(userEmail?: string): BaseRec[] {
  ...
  const inq = safeParse(localStorage.getItem("support:inquiries")).map(
    (r: any) => ({
      id: String(r.id),
      user: r.user || "guest",
      game: r.game,
  ...
  const ref = safeParse(localStorage.getItem("support:refunds")).map(
    (r: any) => ({
      id: String(r.id),
      user: r.user || "guest",
      game: r.game,
  ...
  const all = [...inq, ...ref].filter((r) =>
    userEmail ? r.user === userKey : true
  );
          ...
          >
            ...
            <CardContent className="space-y-3">
              <div className="text-sm">
                유형: <span className="font-medium">{r.kind}</span>
              </div>
              {r.game && (
                <div className="text-sm">
                  게임: <span className="font-medium">{r.game}</span>
                </div>
              )}
              ...
              <div className="text-right">
                <Button
                  ...
                  onClick={() => navigate(`/support01/${r.id}`)}
                >
                  상세보기
                </Button>
              </div>
