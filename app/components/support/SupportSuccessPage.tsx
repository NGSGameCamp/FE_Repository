...
import SplitText from "~/components/y_ui/motion-effects/SplitText";
...
export default function SupportSuccessPage() {
  ...
  const id =
    state.id ||
    `INQ-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
  ...
  return (
    <div className="container mx-auto px-6 py-10 space-y-8">
      ...
      <Card
        className="border-amber-500/30 bg-amber-500/5 mx-auto"
        style={{ maxWidth: 720 }}
      >
        <CardHeader>
        ...
        <CardContent className="text-sm text-muted-foreground space-y-1">
          ...
          <p>
            • 스팸 폴더도 확인해주세요. 답변 이메일이 스팸으로 분류될 수
            있습니다.
          </p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        ...
        <Button onClick={() => nav("/support/my", { replace: true })}>
          나의 문의사항 보기
        </Button>
